...
export function afterInsertAdvancedQuery<S extends PropertiesSchema>(this: Lyra<S>, id: string) {
...
function indexDocument<S extends PropertiesSchema>(docId: string, doc: ResolveSchema<S>, prefix = '') {
...
function indexNumericField<S extends PropertiesSchema>(docId: string, doc: ResolveSchema<S>,  key: string, field: number) {
  ...
  const index = binarySearch(queue.queue, (current => current.priority - field))
