...
t.test('Advanced search with right term' , async t => {
    ...
    const idAlive1 = await insertWithHooks(lyra, {author: 'Alive', 'alive': true, booksSold: 123}) 
    const idAlive2 = await insertWithHooks(lyra, {author: 'Alive', 'alive': true, booksSold: 1234})
    ...
    const resultsDead = advancedSearch(lyra, {where: {alive: false}, term: 'Dead'});
...
t.test('Advanced search with right term but where query produces 1 results' , async t => {
    const lyra = create({
        schema: {
            'author': 'string',
            'alive': 'boolean',
    ...
    await insertWithHooks(lyra, { author: 'Alive', 'alive': false, booksSold: 1233 })
...
t.test('Search with nested properties', async t => {
    ...
    const {id: danieleID} = await insertWithHooks(lyra, {author: {name: "Daniele", alive: true, age: 69}, booksSold: 1233 })
    const {id: antonioID} =  await insertWithHooks(lyra, {author: {name: "Antonio", alive: false, age: 103},  booksSold: 123}) 

    const resultAlive = advancedSearch(lyra, { "term": "Daniele", where: {author: {alive: true}}})
